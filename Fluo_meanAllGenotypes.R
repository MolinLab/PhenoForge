dir.create("plots/plots_meanFluo")


path_plotaverage<-"plots/plots_meanFluo/"

list_allgenos_fluo<-lapply(list_Fluorescence_parameters, function(i){p<-ggplot(i, aes(x=as.Date(`Measuring Date`, format="%Y-%m-%d"), y=value, color=Condition))+
  ...
  theme(axis.text.x = element_text(angle=75, vjust=0.5, size=7), axis.text.y = element_text(size=10), legend.text=element_text(size=7),
...
#ggsave(p,file=paste0(path_plotaverage,paste0(i$variable[1]),"_allGenotypes.png"))
# }
#return(p)
})