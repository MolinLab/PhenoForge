metatable<-cbind(list_Hc_Plant[[1]]$'Plant ID',list_Hc_Plant[[1]]$'Tray Info',
                 list_Hc_Plant[[1]]$'Plant Name')
colnames(metatable)<-c("Plant ID", "Tray Info","Plant Name")
metatable<-as_tibble(metatable)
metatable<-metatable %>% mutate(Condition = if_else(grepl("*DS", metatable$`Tray Info`), 
