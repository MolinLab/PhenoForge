metatable<-cbind(list_Hc_Plant[[4]]$'Plant ID',list_Hc_Plant[[4]]$'Tray Info',
                 list_Hc_Plant[[4]]$'Plant Name')
colnames(metatable)<-c("Plant ID", "Tray Info","Plant Name")
metatable<-as_tibble(metatable)
metatable<-metatable %>% mutate(Condition = if_else(grepl("\\d*DS", metatable$`Tray Info`), 
