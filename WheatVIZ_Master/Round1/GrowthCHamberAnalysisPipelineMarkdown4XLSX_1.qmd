...
list_merged_tables2<-lapply(list_merged_tables2, function(x){x%>%#dplyr::select(-'Measuring Date')%>%
    dplyr::rename('Measuring Date'='Measuring Date.x')})


list_melt_table<-lapply(list_merged_tables2, function(x){
...
list_condition_sd_and_mean<-lapply(list_melt_table, join_and_calc)#calculates mean and sd over all Individuals per group(Condition_mean)
...
#dieser for-loop macht aus EinNameEinIndexAllDatesAllReplicates-file ein mean&sd over replicates per Genotype for each of the two conditions-file in einer List
...
```{r, warning=FALSE, message=FALSE}
path_plotaverage<-"C:/Users/HabisohnC/Desktop/"


select_indices(list_condition_sd_and_mean,"MCARI1-avg")%>%bind_rows()%>%ggplot(aes(x=as.Date(Date, format="%Y-%m-%d"),y=Mean,color=Condition))+
...
select_indices(list_condition_sd_and_mean,"NDVI-avg")%>%bind_rows()%>%ggplot(aes(x=as.Date(Date, format="%Y-%m-%d"),y=Mean,color=Condition))+
...
select_indices(list_condition_sd_and_mean,"PSRI-avg")%>%bind_rows()%>%ggplot(aes(x=as.Date(Date, format="%Y-%m-%d"),y=Mean,color=Condition))+
...
select_indices(list_condition_sd_and_mean,"QY_max")%>%bind_rows()%>%ggplot(aes(x=as.Date(Date, format="%Y-%m-%d"),y=Mean,color=Condition))+xlab("Date")+
  ylab("QY_max average")+
  #geom_rect(aes(xmin=as.Date("2024-02-12", format="%Y-%m-%d"),xmax=as.Date("2024-03-01", format="%Y-%m-%d"),ymin=-Inf,ymax=Inf),alpha=0.02,fill="lightgrey", color="lightgrey")+
  geom_point(size=1.5)+
  theme_bw(base_size = 10)+
...
#Beispiel Lauf für PRI Index von vorn bis hinten:
...
## **All Genotypes over all dates plots**
...
### **Individual Genotypes with water potential plots**

```{r, warning=FALSE, message=FALSE}
for(i in water_tables_all20Genotypes){list_plots_WaterGenotype_QYMax[[length(list_plots_WaterGenotype_QYMax)+1]]<-
 ggplot(i[is.element(i$Date, as.Date(list_QY_max_andNames[[1]]$`Measuring Date`)),],
...
for(i in water_tables_all20Genotypes){list_plots_WaterGenotype_NDVI[[length(list_plots_WaterGenotype_NDVI)+1]]<-
  ggplot(i[is.element(i$Date,as.Date(list_NDVIavg_andNames[[1]]$`Measuring Date`)),],
         aes(x=as.Date(Date, format="%Y-%m-%d"),y=Mean))+xlab("Date")+
  scale_y_continuous(name="Mean water potential [kPa]")+
  #geom_rect(aes(xmin=as.Date("2024-02-12", format="%Y-%m-%d"),xmax=as.Date("2024-03-01", format="%Y-%m-%d"),ymin=-Inf,ymax=Inf),fill="lightgrey",alpha=0.02,color="lightgrey")+
  theme_bw(base_size = 10)+
...
#### **Fluorescence Quantum Yield Maximum individual Genotypes**

```{r,fig.dim=c(6,8),warning=FALSE, message=FALSE}
for(i in list_dual_wpotential_QYMaxGenotype){print(i)}
```

#### **Hyperspectral NDVI individual Genotypes**

```{r,fig.dim=c(6,8),warning=FALSE, message=FALSE}
for(i in list_dual_wpotential_NDVIGenotype){print(i)}
```

#### **Hyperspectral PSRI individual Genotypes**

```{r,fig.dim=c(6,8),warning=FALSE, message=FALSE}
for(i in list_dual_wpotential_PSRIGenotype){print(i)}
```

#### **Hyperspectral MCARI1 individual Genotypes**

```{r,fig.dim=c(6,8),warning=FALSE, message=FALSE}
for(i in list_dual_wpotential_MCARI1Genotype){print(i)}
```
