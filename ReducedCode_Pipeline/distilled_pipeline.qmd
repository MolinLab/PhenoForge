...
ggpairs(dplyr::select(wide_all1803, Condition, ends_with("avg"), QY_max, TG), mapping=ggplot2::aes(colour=Condition),
        lower=list(continous=wrap("smooth", alpha=0.05, size=0.1)),
        diag = list(discrete="barDiag",continuous = wrap("densityDiag", alpha=0.5 )),
        cardinality_threshold = 40, title="Indices,Bonitur,Yield 18.03.", legend=1)+
 ...
 ggsave(filename = paste0(path_plotsDownstream,paste0(wide_all1803$`Measuring Date`[1]),".png"), width=20, height=20)
...
list_Genotypes_2802<-lapply(datasets_Genotypes, function(x){
  ...
  pipi<-dplyr::select(x, Condition, ends_with("avg"), QY_max, TG)
  piplot<-ggpairs(pipi, mapping=ggplot2::aes(colour=Condition),
                                               lower=list(continous=wrap("smooth", alpha=0.05, size=0.1)),
                                               diag = list(discrete="barDiag",continuous = wrap("densityDiag", alpha=0.5 )),
...
####### k means clustering ---loop TODO
 ...
 kmeans_2802<-kmeans(dplyr::select(DS_list_Pattern1$wide_all2802, where(is.numeric)), centers=3, nstart=20 )
 
autoplot(kmeans_2802, data=DS_list_Pattern1$wide_all2802,label = TRUE, label.label = DS_list_Pattern1$wide_all2802$Genotype, label.size=3)+labs(title="3-means clustering 28.02.Gruppe DS Bonitur, Indices, Yield") 
