...
source("RF_forAllCases_Dates_Columns.R", local = knitr::knit_global() )
...
Timepoints_IndexOVERyield_plots<-lapply(Gruppen_timepoints_melt, function(p){
  ...
  theme(axis.text.x = element_text(angle=45, hjust=1.2, vjust=2), axis.text=element_text(size=13), 
...
#Zusammenfügen der dfs wide format(list_cutDFs)
#das ist eine extrem ungute Aufgabe, es sollte eher "Zusammenstöpseln heissen, denn die Durchgänge haben eigentlich alle ein andres Datum....


#my_df <- mutate_if(my_df, is.character, as.factor)

list_cutHc_trimmed<-list_cutHc[-6]
list_cutFc_trimmed<-list_cutFc[-3]

list_numbers<-c(1:length(list_cutFc_trimmed))

doesThatWork<-lapply(list_numbers, function(x){inner_join(list_cutHc_trimmed[[x]], list_cutFc_trimmed[[x]], by="Plant ID")%>%mutate(`Measuring Date.y`=NULL)%>%dplyr::rename("Measuring Date"="Measuring Date.x")})

###frickless verbindet die RGB mit den HCFC dfs, da ,wo das Datum stimmt


####################################diese Funktion läuft, aber sie stellt mir nur die gejoinedten dfs in die Liste, ich will ja auch die, die ohne RGB values sind drin ahben....muss ich nich knobeln...
#######################################


##hier sind wide_formats aller available data für jeweiliges Datum:
#hc, fc, RGB, bonitur in kombination oder alleine....

#ACHTUNG: ich muss die columns der dataframes noch ordnen, bei manchen ist die numerische kolonne unterbrochen... muss ich nohc fixen, aber morgen...

#hc,fc,rgb
wide_all0702<-inner_join(list_cutHc[[1]], list_cutFc[[1]], by="Plant ID")%>%mutate(`Measuring Date.y`=NULL)%>%dplyr::rename("Measuring Date"="Measuring Date.x")%>%inner_join(list_cutRGB_C_hexcodes[[1]], by="Plant ID")%>%mutate(`Measuring Date.y`=NULL)%>%dplyr::rename("Measuring Date"="Measuring Date.x")%>%inner_join(weizenernte, by="Plant ID")%>%dplyr::select(-Tray)%>%inner_join(metatable_slim, by="Plant ID")%>%relocate(Genotype, Condition, .after="Plant ID")



#hc,fc,rgb, bonitur
wide_all0902<-doesThatWork[[2]]%>%inner_join(list_cutRGB_C_hexcodes[[2]], by="Plant ID")%>%mutate(`Measuring Date.y`=NULL)%>%dplyr::rename("Measuring Date"="Measuring Date.x")%>%inner_join(Bonitur_growthCHamber[which(Bonitur_growthCHamber$`Measuring Date`=="2024-02-09"),], by=c("Plant ID", "Measuring Date"))%>%inner_join(weizenernte, by="Plant ID")%>%dplyr::select(-GenotypeNr,-Tray.x,-Tray.y, -GeneralImpression)%>%relocate(Genotype, .after = `Plant ID`)%>%relocate(Condition, .after = Genotype)


wide_all1202<-inner_join(list_cutFc[[3]], list_cutRGB_C_hexcodes[[3]], by="Plant ID", "Measuring Date")%>%dplyr::select(-`Measuring Date.y`)%>%dplyr::rename("Measuring Date"="Measuring Date.x")%>%inner_join(metatable_slim, by="Plant ID")%>%relocate(Genotype, Condition, .after="Plant ID")

wide_all1402<-doesThatWork[[3]]%>%inner_join(list_cutRGB_C_hexcodes[[4]], by="Plant ID")%>%mutate(`Measuring Date.y`=NULL)%>%dplyr::rename("Measuring Date"="Measuring Date.x")%>%inner_join(Bonitur_growthCHamber[which(Bonitur_growthCHamber$`Measuring Date`=="2024-02-14"),], by=c("Plant ID", "Measuring Date"))%>%inner_join(weizenernte, by="Plant ID")%>%dplyr::select(-GenotypeNr, -Tray.x, -Tray.y, -GeneralImpression)%>%dplyr::relocate(Genotype, Condition, .after=`Plant ID`)




wide_all2102<-doesThatWork[[4]]%>%inner_join(list_cutRGB_C_hexcodes[[5]], by="Plant ID")%>%mutate(`Measuring Date.y`=NULL)%>%dplyr::rename("Measuring Date"="Measuring Date.x")%>%inner_join(Bonitur_growthCHamber[which(Bonitur_growthCHamber$`Measuring Date`=="2024-02-23"),], by=c("Plant ID"))%>%mutate(`Measuring Date.y`=NULL)%>%dplyr::rename("Measuring Date"="Measuring Date.x")%>%inner_join(weizenernte, by="Plant ID")%>%dplyr::select(-Tray.x, -Tray.y, -GenotypeNr,-GeneralImpression)%>%relocate(Genotype, Condition, .after=`Plant ID`)



wide_all2302<-doesThatWork[[5]]%>%inner_join(metatable_slim, by="Plant ID")%>%relocate(Genotype, Condition, .after="Plant ID")%>%inner_join(weizenernte, by="Plant ID")%>%dplyr::select(-Tray)
#wide_all2602<-doesThatWork[[6]]

wide_all2602<-list_cutHc[[6]]%>%inner_join(metatable_slim, by="Plant ID")%>%relocate(Genotype, Condition, .after="Plant ID")

wide_all2802<-doesThatWork[[6]]%>%inner_join(list_cutRGB_C_hexcodes[[6]], by="Plant ID")%>%mutate(`Measuring Date.y`=NULL)%>%dplyr::rename("Measuring Date"="Measuring Date.x")%>%inner_join(Bonitur_growthCHamber[which(Bonitur_growthCHamber$`Measuring Date`=="2024-02-23"),], by=c("Plant ID"))%>%mutate(`Measuring Date.y`=NULL)%>%dplyr::rename("Measuring Date"="Measuring Date.x")%>%inner_join(weizenernte, by="Plant ID")%>%relocate(Genotype, Condition, .after="Plant ID")%>%dplyr::select(-Tray.y,-Tray.x,-GenotypeNr, -GeneralImpression)

wide_all0103<-doesThatWork[[7]]%>%inner_join(metatable_slim, by="Plant ID")%>%relocate(Genotype, Condition, .after="Plant ID")%>%inner_join(weizenernte, by="Plant ID")%>%dplyr::select(-Tray)

wide_all0403<-doesThatWork[[8]]%>%inner_join(metatable_slim, by="Plant ID")%>%relocate(Genotype, Condition, .after="Plant ID")%>%inner_join(weizenernte, by="Plant ID")%>%dplyr::select(-Tray)

wide_all0603<-doesThatWork[[9]]%>%inner_join(list_cutRGB_C_hexcodes[[7]], by="Plant ID")%>%mutate(`Measuring Date.y`=NULL)%>%dplyr::rename("Measuring Date"="Measuring Date.x")%>%inner_join(weizenernte, by="Plant ID")%>%dplyr::select(-Tray)%>%inner_join(metatable_slim, by="Plant ID")%>%relocate(Genotype, Condition, .after="Plant ID")


wide_all0803<-doesThatWork[[10]]%>%inner_join(metatable_slim, by="Plant ID")%>%relocate(Genotype, Condition, .after="Plant ID")%>%inner_join(weizenernte, by="Plant ID")%>%dplyr::select(-Tray)

wide_all1103<- doesThatWork[[11]]%>%inner_join(metatable_slim, by="Plant ID")%>%relocate(Genotype, Condition, .after="Plant ID")%>%inner_join(weizenernte, by="Plant ID")%>%dplyr::select(-Tray)

wide_all1303<-doesThatWork[[12]]%>%inner_join(list_cutRGB_C_hexcodes[[8]], by="Plant ID")%>%mutate(`Measuring Date.y`=NULL)%>%dplyr::rename("Measuring Date"="Measuring Date.x")%>%inner_join(Bonitur_growthCHamber[which(Bonitur_growthCHamber$`Measuring Date`=="2024-03-13"),], by=c("Plant ID", "Measuring Date"))%>%inner_join(weizenernte, by="Plant ID")%>%relocate(Genotype, Condition, .after="Plant ID")%>%dplyr::select(-Tray.y, -Tray.x, -GeneralImpression, -GenotypeNr)


wide_all1503<-doesThatWork[[13]]%>%inner_join(metatable_slim, by="Plant ID")%>%relocate(Genotype, Condition, .after="Plant ID")%>%inner_join(weizenernte, by="Plant ID")%>%dplyr::select(-Tray)

wide_all1803<-doesThatWork[[14]]%>%inner_join(metatable_slim, by="Plant ID")%>%relocate(Genotype, Condition, .after="Plant ID")%>%inner_join(weizenernte, by="Plant ID")%>%dplyr::select(-Tray)


Pattern1<-grep("wide",names(.GlobalEnv),value=TRUE)
Pattern1_list<-do.call("list",mget(Pattern1))
#mit diesen wide tables kann ich alle PCAs, lms, kmeans clustering etc machen....
#was brauche ich für die scatterplots...auch wide glaub ich--JA
...
path_plotsDownstream<-"C:/Users/HabisohnC/Desktop/DigAB/plots/DownstreamStatistics_plots/"
 ...
 ################### looping
 #liste der wide_dfs, die HC UND FC UND Yield haben: != list_Pattern1 !! NOTE
 list_scatterplotting<-list(wide_all0702,wide_all0902, wide_all1402, wide_all2102, wide_all2302, wide_all2802, wide_all0103, wide_all0403, wide_all0603, wide_all0803, wide_all1303, wide_all1503, wide_all1803)
 
 
#######der loop läuft noch nicht :((((.......))))
 testrunscatter<-lapply(list_scatterplotting, function(i){
   
   newdf<-dplyr::select(i, Condition, ends_with("avg"), QY_max, TG)
...
####loop over Genotypes für den 2802
 
path_plotsDownstream<-"C:/Users/HabisohnC/Desktop/DigAB/plots/DownstreamStatistics_plots/testplots/"

datasets_Genotypes<-lapply(list_of_names, function(x){result<-dplyr::filter(wide_all2802, Genotype==x)
return(result)})
names(datasets_Genotypes)<-list_of_names

datasets_Genotypes<-datasets_Genotypes[-c(7,13)]###schmeissen mir meinen loop auf, weil da irgendwas nicht passt drin...


list_Genotypes_2802<-lapply(datasets_Genotypes, function(x){
  
  pi<-dplyr::select(x, Condition, ends_with("avg"), QY_max, LeafNumber:TG)
  piplot<-ggpairs(pi, mapping=ggplot2::aes(colour=Condition),
                                               lower=list(continous=wrap("smooth", alpha=0.05, size=0.1)),
                                               diag = list(discrete="barDiag",continuous = wrap("densityDiag", alpha=0.5 )),
                                               cardinality_threshold = 40, title=paste0(x$Genotype[1]," Indices,Bonitur,Yield 28.02."), legend=1)+
    theme(panel.grid.major = element_blank())+
    scale_color_manual(values = c("Control"="turquoise3", 
                                  "Drought stress"="coral"))+
    scale_fill_manual(values = c("Control"="turquoise3", 
...
#lapply(list_Genotypes_2802, function(x){
 # ggsave(x,filename = paste0(path_plotsDownstream,paste0(x$title ,"_2nd.png")), width=20, height=20)
#})



###gibt mir den scatterplot pro Genotyp für den 28.02. aber ich habe die BoniturDaten vergessen, mit rein zu nehmen :/ -> morgen nochmal!!
```
...
list_PCA_wideDFs<-lapply(Pattern1_list, function(x){
...
list_biplots_wideDFs<-lapply(list_PCA_wideDFs, ggbiplot::ggbiplot)
  

################
#auftrennen der dfs nach Condition: hier DS


 DS_list_Pattern1<-lapply(Pattern1_list, function(g){filter(g, Condition=="Drought stress")})
 
 
 
 list_PCA_wideDFs_DS_Date<-lapply(DS_list_Pattern1, function(x){
  gh<-dplyr::select(x, where(is.numeric))
  y<-gh[,which(apply(gh, 2, var)!=0)]
   p<-prcomp(dplyr::select(y, where(is.numeric)), scale.=TRUE)
 ...
 list_biplots_wideDFs_DS_Date<-lapply(list_PCA_wideDFs_DS_Date, function(biplotting){ggbiplot::ggbiplot(biplotting,labels = DS_list_Pattern1[[1]]$Genotype, groups = DS_list_Pattern1[[1]]$Genotype)+labs(title=paste0("PCA biplot ", biplotting$date))})
 
 
# lapply(list_biplots_wideDFs_DS_Date, function(pi){
 #  ggsave(pi,filename = paste0(path_plotsDownstream,paste0(pi$labels$title,".png")), width=20, height=20)
# })
```

```{r}
####### k means clustering ---loop TODO
DS_wide2802<-DS_list_Pattern1$wide_all2802
pinky<-dplyr::select(DS_wide2802, where(is.numeric))

kmeans_2802<-kmeans(pinky, centers=4, nstart=20 )
 

DS_wide2802$cluster<-kmeans_2802$cluster

autoplot(kmeans_2802, data=pinky,label = TRUE, label.label = DS_list_Pattern1$wide_all2802$Genotype, label.size=3)+labs(title="3-means clustering 28.02.Gruppe DS Bonitur, Indices, Yield") 

ggsave(filename =paste0(path_plotsDownstream,"clusters1.png"),width=10, height=10 )

sil <- silhouette(kmeans_2802$cluster, dist(DS_list_Pattern1$wide_all2802))

fviz_silhouette(sil)

ggsave(filename =paste0(path_plotsDownstream,"silhouette1.png"),width=15, height=15 )
```
