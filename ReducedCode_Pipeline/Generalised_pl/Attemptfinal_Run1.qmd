...
source("datawrangling2.R", local = knitr::knit_global() )
...
#removed the water duo plots, water information will be plotted in meta-plot-\>TODO!!!!!!
...
die source files muss ich noch checken, ob ich die nicht früher schon wo dazu tun könnte, bei obigem source Teil ...
...
list_PCAplots_NDVI_Size_MCARI1_TG<-lapply(CamData_Bonitur_TG, function(x){
  ...
  plottings<-ggbiplot::ggbiplot(p, groups=x$Condition, colour=x$Condition, fill=X$Condition, ellipse = TRUE, circle = FALSE)+labs(title=p$date)+
...
ggplot(Mean_emmean_Groups, aes(x=Date, y=Mean, colour=Condition, group=Condition))+
...
ggplot(Mean_emmean_Groups_Size, aes(x=Date, y=Mean, colour=Condition, group=Condition))+
  geom_point()+geom_line()+
  geom_errorbar(aes(ymin=Mean-SD, ymax=Mean+SD), width=.2)+
...
ggplot(Mean_emmean_Groups_MCARI1, aes(x=Date, y=Mean, colour=Condition, group=Condition))+
  geom_point()+geom_line()+
  geom_errorbar(aes(ymin=Mean-SD, ymax=Mean+SD), width=.2)+
...
ggplot(Mean_emmean_Groups_BBCH, aes(x=Date, y=Mean, colour=Condition, group=Condition))+
  geom_point()+geom_line()+
  geom_errorbar(aes(ymin=Mean-SD, ymax=Mean+SD), width=.2)+
...
Group specific plotting of emmeans; verschiedene Varianten, alle nicht sehr gut....

```{r}
#ich brauche  doch gaR  keine signifikanzlevels mehr zwischen den GRuppen, da meine Anova doch schon gezeigt hat, dass es da einen sign.<unterschied gibt, oder??????


 ggboxplot(list_anovas_NDVI_allPosthocs[[8]], x=".group", y="emmean", color="Condition", facet.by = ".group")+stat_compare_means(method="anova")+
   stat_compare_means(label = "p.signif", method = "t.test",
                     ref.group = ".all.")
 
 
 compare_means(emmean~.group, data=list_anovas_NDVI_allPosthocs[[7]])
 
 ggboxplot(list_anovas_NDVI_allPosthocs[[7]], x=".group", y="emmean", color="Condition")+geom_text(aes(label=Genotype),size=2.5)

 #filter the groups with only one observations from frames:
 #aber:wollen wir das??????? wir wollen ja sehen, wer von wem unterschiedlich ist!!
 ...
 #also ich muss die Gruppen mit nur einem Bewohner entferne, da ich aus einem Wert keinen p-value berechnen kann über die distribution der Werte......
 
 grouped_ggbetweenstats(data=list_anovas_NDVI_allPosthocs[[8]] %>%group_by(.group) %>% filter(n() > 1) %>%ungroup(),x=.group, y=emmean,grouping.var=Condition )
```
