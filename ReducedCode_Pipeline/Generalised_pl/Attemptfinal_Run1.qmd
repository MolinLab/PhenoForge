...
for(i in 1:length(list_mean_sd_INDICES)){
  ...
  ggsave(file=paste0(path_plotaverage, paste0(list_mean_sd_INDICES[[i]]$Name[1], paste0(list_mean_sd_INDICES[[i]]$Index[1], ".png"))))
...
boxplots_path<-"plots/boxplots/HC_TG_sep/"
...
boxplots_path<-"plots/boxplots/DS_only/"
...
boxplots_path<-"plots/boxplots/sep_indices/"
...
boxes_TGscore<-
  lapply(CamData_HConly_melt, function(yi){
    if(!is.null(yi)){
      ...
      boxa<-ggplot(ya, aes(x=variable, y=value,color=TG_score))+geom_boxplot(outlier.size = 0.5)+ylim(min(ya$value), max(ya$value))+
        labs(title=paste0("Drought stress ", yi$`Measuring Date`[1]))+
        theme(axis.text.x = element_text(angle=90, vjust=0.5, size=9))
      
      yb<-yi%>%dplyr::filter(Condition=="Control")%>%dplyr::filter(variable=="NDVI-avg"| variable=="OSAVI-avg")
      
      boxb<-ggplot(yb, aes(x=variable, y=value,color=TG_score))+geom_boxplot(outlier.size = 0.5)+ylim(min(ya$value), max(ya$value))+
        labs(title=paste0("Control ", yi$`Measuring Date`[1]))+
        theme(axis.text.x = element_text(angle=90, vjust=0.5, size=9))
      ...
      ggsave(boxi, filename=paste0(boxplots_path, paste0( yi$`Measuring Date`[1],"_DSandC_box_NDVIOSAVISIPI.png" )))
...
boxes_TGscore<-
  lapply(CamData_HConly_melt, function(yi){
    if(!is.null(yi)){
      ...
      ggsave(boxi, filename=paste0(boxplots_path, paste0( yi$`Measuring Date`[1],"_DSandC_boxMCARI.png" )))
...
boxes_TGscore<-
  lapply(CamData_HConly_melt, function(yi){
    if(!is.null(yi)){
      ...
      ggsave(boxi, filename=paste0(boxplots_path, paste0( yi$`Measuring Date`[1],"_DSandC_boxNDVI2PRIPSRI.png" )))
...
boxplots_path_cond<-"plots/boxplots/index/"
...
boxplots_path_cond2<-"plots/boxplots/index/"
...
boxplots_path_allInd<-"plots/boxplots/index/"
...
boxplots_path_allInd<-"plots/boxplots/whichplots/"
...
lapply(list_anovas_NDVI_allPosthocs, function(z){
  ...
  print(ggplot(z, aes(x=.group, y=emmean, color=Condition))+geom_boxplot()+labs(title= z$Date[1],x="group")+
