...
Date_format<-format(as.POSIXct(tempandothermeta$Time,
                               format = '%m/%d/%Y %H:%M:%S'),
                    format = '%Y-%m-%d')

tempandothermeta<-tempandothermeta%>%
  mutate(Date=ymd(Date_format))%>%relocate(Date)
...
metad_maxT<-meta_data_mean%>%group_by(Date)%>%
  summarise(dplyr::across(c(Tempsum,RHsum),max))%>%
