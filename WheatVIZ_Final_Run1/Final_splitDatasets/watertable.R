...
water_readings_DSOnly<-water_readings2%>%dplyr::select(contains("DS"))%>%
  mutate(Date=water_readings2$Date)%>%relocate(Date)

water_readings_DSOnly[,2:ncol(water_readings_DSOnly)]<-
  sapply(water_readings_DSOnly[,2:ncol(water_readings_DSOnly)], as.numeric)
