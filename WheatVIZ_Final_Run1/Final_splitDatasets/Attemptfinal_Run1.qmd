...
all5
ggsave(file="plots/metaData.png")
```
...
source("recreate_wholeDataset.R", local = knitr::knit_global() )


```

#Einschub:cutting für das split dataset...

```{r}
CamData_Bonitur_TG<-lapply(CamData_Bonitur_TG, function(cut){
...
CamData_HConly_melt<-lapply(CamData_HConly_melt, function(xix){
  
  if(xix$`Measuring Date`[1] <= "2024-02-20"){
    
    xix_new<-xix%>%dplyr::filter(variable!= "MCARI1-avg")%>%dplyr::filter(variable!= "OSAVI-avg")
    return(xix_new)
...
boxes_TGscore<-
  lapply(CamData_HConly_melt, function(yi){
    if(!is.null(yi)){
     ...
     if(str_contains(unique(yi$variable), "OSAVI-avg")){
      ...
      ggsave(boxi, filename=paste0(boxplots_path, paste0( yi$`Measuring Date`[1],"_DSandC_NDVISIPI.png" )))
...
boxes_TGscore<-
  lapply(CamData_HConly_melt, function(yi){
    if(!is.null(yi)){
     ...
     if(str_contains(unique(yi$variable), "MCARI1-avg")){
...
list_PCAplots_NDVI_Size_MCARI1_TG<-lapply(CamData_Bonitur_TG, function(x){
  if(str_contains(colnames(x), "MCARI1-avg")){
  ...
  gh<-dplyr::select(x,`NDVI-avg`, `Size-avg`,TG)
  y<-gh[,which(apply(gh, 2, var)!=0)]
  p<-prcomp(dplyr::select(y, where(is.numeric)), scale.=TRUE)
  p$date<-x$`Measuring Date`[1]
  ...
  scale_fill_manual(name="Condition", values=c( "turquoise","coral"))
   ...
   ggsave(plottings, filename=paste0(path_PCA, paste0( x$`Measuring Date`[1],"NDVI_Size.png" )))
...
dir.create("plots/PCA/NoTG")

path_PCA<-"plots/PCA/NoTG/"

list_PCAplots_NDVI_Size_MCARI1_TG<-lapply(CamData_Bonitur_TG, function(x){
  if(str_contains(colnames(x), "MCARI1-avg")){
  
  gh<-dplyr::select(x,`NDVI-avg`, `Size-avg`,`MCARI1-avg`)
  y<-gh[,which(apply(gh, 2, var)!=0)]
  p<-prcomp(dplyr::select(y, where(is.numeric)), scale.=TRUE)
  p$date<-x$`Measuring Date`[1]
  ...
  gh<-dplyr::select(x,`NDVI-avg`, `Size-avg`)
  y<-gh[,which(apply(gh, 2, var)!=0)]
  p<-prcomp(dplyr::select(y, where(is.numeric)), scale.=TRUE)
  p$date<-x$`Measuring Date`[1]
...
lapply(list_anovas_NDVI_allPosthocs, function(z){
  ...
  one<-ggplot(z, aes(x=.group, y=emmean, color=Condition))+geom_boxplot()+labs(title=paste0( z$Date[1], " NDVI"),x="group")+
  ...
  ggsave(one, filename=paste0(posthoc_path, paste0(z$Date[1]), "posthocNDVI.png"))
   ...
   return(one)
    ...
    two<-ggplot(z, aes(x=.group, y=emmean, color=Condition))+geom_boxplot()+labs(title=paste0( z$Date[1], " NDVI"), x="group")+
 scale_colour_manual(values = c("Control"="turquoise3", 
                                "Drought stress"="coral"))+theme_bw()
