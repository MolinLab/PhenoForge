...
join_and_calc<-function(x,y){
  x%>%
    inner_join(dplyr::select(y, -'Tray Info'),
...
merge_and_select<-function(x,y){
  x%>%merge(y, by="Plant ID", all=TRUE)%>%dplyr::select(- 'Tray Info')}
...
fuse_me_Bonitur<-function(df_split, extra_data){
  ...
  data_merged<-lapply(extraData_list, function(x){
    ...
    if(df_split_date == x_date | df_split_date +1 == x_date){
      x$`Measuring Date`<-df_split$`Measuring Date`
      inner_join(df_split, x, by=c("Measuring Date", "Plant ID", "Genotype", "Condition"))
    }
    else {
      return(NULL)
    }
